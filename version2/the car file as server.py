...
# creating the sensors/motors variables
kit = MotorKit(i2c = board.I2C())
distance_sensor = DistanceSensor(echo=17,trigger=4)

# reseting the sensors/motors values for better performance
...
def handle_client_message(conn, cmd, data):
    ...
    if cmd == PROTOCOL_CLIENT["log in"]:
        ...
        try:
            ...
            print("Error:",e)
    elif cmd == PROTOCOL_CLIENT["get distance"]:
        distance = get_distance()
        build_and_send_message(conn,PROTOCOL_SERVER["return distance"],str(distance)[:5])
    else:
        pass
    
# other funtions
def move(left_throttle,right_throttle): # for motors
