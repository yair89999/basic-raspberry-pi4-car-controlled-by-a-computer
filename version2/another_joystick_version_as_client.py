...
def recv_message_and_parse(conn):
    ...
    print(f"[SERVER] {str(conn.getpeername())}",full_msg)
...
def main():
    ...
    logged_in = False
    while i < 100:
        build_and_send_message(using_socket,PROTOCOL_CLIENT["log in"],server_code)
    ...
    iterations = 0 # when it can be divided by 10 it will send a get distance request
    distance = None
    while game_on:
        clock.tick(60)
        ...
        if iterations % 10 != 0 and iterations % 15 != 0:
        ...
        elif iterations % 15 == 0:
            build_and_send_message(using_socket,PROTOCOL_CLIENT["i am here"],"")
        elif iterations % 10 == 0:
            build_and_send_message(using_socket,PROTOCOL_CLIENT["get distance"],"")
            recv_message_and_parse(using_socket)
            
            cmd,data = recv_message_and_parse(using_socket)
            print(cmd,data)
            if cmd == PROTOCOL_SERVER["return distance"]:
                try:
                    distance = int(data)
                except:
                    pass
        ...
        if distance == None:
            distance_to_show = "__.__"
        window.blit(font.render(str(distance_to_show),True,(0,0,0)),(10,10))
...
if __name__ == '__main__':
    try:
        main()
    except ConnectionAbortedError: # usually raised because the server closes the socket because it didnt get a I_AM_HERE message
        main()
